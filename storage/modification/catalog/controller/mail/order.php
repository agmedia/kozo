	...
	public function add($order_info, $order_status_id, $comment, $notify) {
		...
		foreach ($order_products as $order_product) {
			...
			foreach ($order_options as $order_option) {
				...
				$option_data[] = array('sku' => $order_option['sku'],
					'name'  => $order_option['name'],
					'value' => (utf8_strlen($value) > 20 ? utf8_substr($value, 0, 20) . '..' : $value) . ($order_option['sku']==''?'':' (sku:'.$order_option['sku'].')' )
				);
			}

			
				$querysku = $this->db->query("SELECT sku FROM " . DB_PREFIX . "product WHERE  model = '" . $this->db->escape($order_product['model']) . "'");
	$psku = $querysku->row;
			$data['products'][] = array('sku' => $psku['sku'],
