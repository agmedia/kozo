...
$data['custom_alt'] = $product_info['custom_alt'];
		...
		'new_label'  => $is_new,
				...
				$data['text_soldout'] = $this->language->get('text_soldout');
				...
				if($data['product_variant_status']){
					...
					if($data['variant_setting']['type'] == 'custom') {
						$product_variant = $this->model_catalog_product->getProductVariant($product_id);
					} else {
						if(isset($product_info[$data['variant_setting']['type']]) && !empty($product_info[$data['variant_setting']['type']])){
							$filters = array(
								'filter_name' => $product_info[$data['variant_setting']['type']]
							);
							$products = $this->model_catalog_product->getProducts($filters);
							if(count($products) > 1) {
								$product_variant = array_column($products, 'product_id');
							}
						}
					}
					...
					if($product_variant){
						...
						if (isset($data['variant_setting']['variant_text'][$language_id])){
							$data['variant_setting']['variant_text'] = $data['variant_setting']['variant_text'][$language_id];
						} else {
							$data['variant_setting']['variant_text'] = array_shift($data['variant_setting']['variant_text']);
						}
						...
						foreach($product_variant as $product){
							...
							if($product_info){	
								...
								if ($data['variant_setting']['quantity'] == 0 && $product_info['quantity'] <= 0) {
									continue;
								} elseif ($data['variant_setting']['quantity'] == 1 && $product_info['quantity'] <= 0) {
									$variants[$product]['stock'] = 0;
								} else {
									$variants[$product]['stock'] = 1;
								}
									
								if(isset($variant_images[$product]) && !empty($variant_images[$product])){
									$variant_image = $product_info['image'];
								} else {
									$variant_image = $product_info['image'];
								}
								
								$width = ($data['variant_setting']['image_width']) ? $data['variant_setting']['image_width'] : 30;
								$height = ($data['variant_setting']['image_width'])?$data['variant_setting']['image_height']:30;
								
								$variants[$product]['variant_image'] = $this->model_tool_image->resize($variant_image, $width, $height);
																								
						
								$variants[$product]['name'] = $product_info['name'];
								$variants[$product]['href'] = $this->url->link('product/product', 'product_id=' . $product);
								$variants[$product]['active'] = ($this->request->get['product_id'] === $product) ? 1 : 0;							
...
$this->load->model('extension/fbecommevnt');
			$data['fbecommevnt_viewprod'] = $this->model_extension_fbecommevnt->viewprod(isset($this->request->get['product_id']) ? $this->request->get['product_id'] : 0);
			if(!empty($data['fbecommevnt_viewprod'])) {
			$data['footer'] = $data['fbecommevnt_viewprod'] . $data['footer'];
			}
				$data['header'] = '';
				$data['is_ajax'] = 1;
			} else {
