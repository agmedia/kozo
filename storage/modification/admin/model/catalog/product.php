		...
		if (isset($data['product_attribute'])) {
			foreach ($data['product_attribute'] as $product_attribute) {
				if ($product_attribute['attribute_id']) {
					...
					if (count($product_attribute['preset_id']) >= 1 && $this->config->get('module_atpresets_installed')==1) {
						...
						$presets_text = $this->model_extension_atpresets_atpresets->getManyPresetsTexts($product_attribute['preset_id']);
				...
				if ($this->config->get('module_atpresets_installed')==1) {
					...
					if ($are_same) { 
						foreach ($product_attribute['preset_id'] as $preset_id) {
							if ($preset_id>0)
								$this->db->query("INSERT INTO " . DB_PREFIX . "product_attribute SET preset_id = '".$preset_id."', product_id = '" . (int)$product_id . "', attribute_id = '" . (int)$product_attribute['attribute_id'] . "', language_id = '" . (int)$language_id . "', text = '" .  $this->db->escape($presets_text[$preset_id][$language_id]) . "'");
						}						
					} else {
			...
			foreach ($data['product_option'] as $product_option) {
				...
				if($this->config->get('product_variant_setting_status')){ 
					if (isset($data['product_variant'])) {
						array_unshift($data['product_variant'], $product_id);
					} else {
						$data['product_variant'] = array($product_id);
					}
					
					$data['product_variant'] = array_filter($data['product_variant']);
					$find = "";
					foreach($data['product_variant'] as $product){					
						$find .= " FIND_IN_SET('" . $product . "',`variant_id`) OR";					
					}
					$find = rtrim($find,"OR");
					$this->db->query("DELETE FROM " . DB_PREFIX . "product_variant WHERE " . $find);
					if(count($data['product_variant'])>1){
						
						$variant = implode(",", $data['product_variant']);
					
						$this->db->query("INSERT INTO " . DB_PREFIX . "product_variant SET variant_id = '" . $variant . "'");
					}
					
					if (isset($data['product_variant_image'])) {
						$this->load->model('setting/setting');
					    $variant_image = $this->model_setting_setting->getSetting('product_variant');
						
						$variant_images = $variant_image;
						
						$variant_images['product_variant_image'][$product_id] = $data['product_variant_image'];
						
						$this->model_setting_setting->editSetting('product_variant', $variant_images);		
	...
	public function getProductLayouts($product_id) {
		...
		return $product_layout_data;
				...
				public function getProductVariant($product_id) {
						...
						$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "product_variant WHERE FIND_IN_SET('" . (int)$product_id . "',`variant_id`)");
						
						if($query->row){
							$products = explode(",",$query->row['variant_id']);
