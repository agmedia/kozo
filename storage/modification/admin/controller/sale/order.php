...
use Agmedia\Helpers\File\Excel;
use Agmedia\Models\Order\Order;
use Agmedia\Models\Order\OrderService;
...
class ControllerSaleOrder extends Controller {
	...
	if (isset($this->request->get['filter_payment_method'])) {
        ...
        $data['shipping_intereuropa_excel'] = $this->url->link('sale/order/intereuropa_xls', 'user_token=' . $this->session->data['user_token'], true);
	...
	'filter_payment_method'   => $filter_payment_method,	
		...
		foreach ($results as $result) {
		    $shipping = null;
		    //$dhl = DHL::hasLabel($result['order_id']);
		    
		    if ($result['intereuropa']) {
		        $shipping = 'intereuropa';
            }
		    /*if ($dhl) {
                $shipping = 'dhl';
            }*/
	...
	'payment_method'      => $result['payment_method'],	
                ...
                'order_service' => $result['order_service'],
				'order_status'  => $result['order_status'] ? $result['order_status'] : $this->language->get('text_missing'),
				'shipping'      => $shipping,
                'paid_order'    => $result['paid_order'],
		...
		\Agmedia\Helpers\Log::write($data['orders'], 'data_orders');
	...
	if (isset($this->request->get['filter_payment_method'])) {
		$url .= '&filter_payment_method=' . $this->request->get['filter_payment_method'];
            ...
            // fj.agmedia.hr
            $data['order_service'] = OrderService::info($this->request->get['order_id']);
            $data['paid_order'] = $order_info['paid_order'];
	...
	'sku'  => $option['sku'],
				...
				$querysku = $this->db->query("SELECT sku FROM " . DB_PREFIX . "product WHERE  product_id = '" . (int)$product['product_id'] . "'");
	$psku = $querysku->row;
				$data['products'][] = array(
					'order_product_id' => $product['order_product_id'],'sku'    => $psku['sku'],
					'product_id'       => $product['product_id'],
					'name'    	 	   => $product['name'],
					'model'    		   => $product['model'],
					'option'   		   => $option_data,
     ...
     * @return mixed
     */
    public function intereuropa_xls()
    {
        $json = array();
        ...
        if (!$this->user->hasPermission('modify', 'sale/order')) {
            ...
            $orders = Order::whereIn('order_id', collect($this->request->post['selected'])->flatten())->get()->toArray();
            
            if (empty($orders)) {
                return $this->asJson(['error' => 'Nema podataka!']);
            }
            
            $excel = new Excel($orders, 'intereuropa');
            
            Order::whereIn('order_id', collect($this->request->post['selected'])->flatten())->update([
                'intereuropa' => 1
            ]);
            
            return $excel->make('intereuropa')->response('stream');
        }
    ...
    public function sendToErp() {
        ...
        if (!$this->user->hasPermission('modify', 'sale/order')) {
            ...
            $service = new Service();
            $order   = Order::where('order_id', $order_id)->with('products', 'totals', 'status')->first()->toArray();
            
            \Agmedia\Helpers\Log::info($order, 'order_' . $order_id);
            
            $new_order = new Order();
            $response  = $service->sendOrder($new_order->makeRequest($order), $new_order->resolveTarget($order));
            
            \Agmedia\Helpers\Log::info($response, 'order_' . $order_id);
            
            OrderService::storeResponse($response, $order_id);
            
            if ($response) {
                $json['invoice_no'] = $response;
            } else {
                $json['error'] = $this->language->get('error_action');
            }
        }
    ...
    public function orderPaid()
    {
        ...
        if (!$this->user->hasPermission('modify', 'sale/order')) {
            ...
            $order = Order::where('order_id', $this->request->get['order_id'])->first();
            ...
            $checked = 0;
            if ($this->request->get['checked'] == 'true') {
                $checked = 1;
            }
            
            Order::where('order_id', $this->request->get['order_id'])->update([
                'paid_order' => $checked
            ]);
            
            return $this->asJson([200 => 'success']);
        }
