	...
	protected function getForm() {
		...
		if ($this->config->get('module_atpresets_installed')==1) {
			...
			if (isset($this->request->get['attribute_id']) && ($this->request->server['REQUEST_METHOD'] != 'POST')) {
				...
				$all_presets = $this->model_extension_atpresets_atpresets->getPresets(array('attribute_id' => $this->request->get['attribute_id']));			
				
				foreach ($all_presets as $preset) {
					$presets[] = $this->model_extension_atpresets_atpresets->getPreset($preset['preset_id']);	
				}
			}
			foreach ($presets as $key=>$preset) {
				$presets[$key]['text_short'] = ((utf8_strlen($preset['text'])>50)?substr($preset['text'], 0, 50).'...':$preset['text']);
			}
