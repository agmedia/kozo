...
class ControllerCommonLogin extends Controller {
	...
	public function index() {
		...
		if (isset($_GET['cron']) && ($_GET['cron'] == 1) && $this->config->get('advso' . $_GET['user_id'] . 'cron_setting')) {
			$cron_settings = unserialize($this->config->get('advso' . $_GET['user_id'] . 'cron_setting'));
			foreach ($cron_settings as $cron_setting) {
            	if (isset($_GET['cron_token']) && ($_GET['cron_token'] == $cron_setting['cron_token']) && isset($_GET['cron_id']) && ($_GET['cron_id'] == $cron_setting['cron_id'])) {
					...
					$this->response->redirect($this->url->link($cron_route, 'user_id='.$cron_setting['cron_user_id'].'&user_token='.$this->session->data['user_token'].'&cron_id='.$cron_setting['cron_id'].'&cron=2', 'SSL'));
				}
			}
		}
            

		if (isset($_GET['cron']) && ($_GET['cron'] == 1) && $this->config->get('advpp' . $_GET['user_id'] . 'cron_setting')) {
			$cron_settings = unserialize($this->config->get('advpp' . $_GET['user_id'] . 'cron_setting'));
			foreach ($cron_settings as $cron_setting) {
            	if (isset($_GET['cron_token']) && ($_GET['cron_token'] == $cron_setting['cron_token']) && isset($_GET['cron_id']) && ($_GET['cron_id'] == $cron_setting['cron_id'])) {
					$this->session->data['user_token'] = md5(mt_rand());
					$this->request->get['user_token'] = $this->session->data['user_token'];
